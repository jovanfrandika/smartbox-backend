...
func Deliver(r *chi.Mux, usecase u.Usecase) {
	...
	(*r).With(jwt.AuthMiddleware).Get("/pick-ups", d.GetNearbyPickUps)
	...
	(*r).With(jwt.AuthMiddleware).Post("/code/send", d.SendParcelCode)
	...
	(*r).With(jwt.AuthMiddleware).Post("/close", d.CloseDoor)
}
